...
bool OSystem_iOS7::isiOSAppOnMac() const {
	...
	execute_on_main_thread(^ {
		iPhoneView *view = [iOS7AppDelegate iPhoneView];
		if ([view respondsToSelector:@selector(isiOSAppOnMac)]) {
			isiOSAppOnMac = [view isiOSAppOnMac];
		}
	});
