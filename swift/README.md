# ScummVM Swift package

SwiftUI wrapper around the upstream ScummVM codebase, packaged as a Swift Package with minimal Objective-C++ glue. The package reuses the shared upstream submodule while keeping Apple-specific wrapper code under this directory.

Upstream ScummVM repository: [scummvm/scummvm](https://github.com/scummvm/scummvm).

## Goals
- Reuse upstream ScummVM code directly via the repository's `scummvm/` git submodule.
...
- Ship a Swift Package that can be embedded in iOS, tvOS, and macOS apps.
...
**Source mode (`SCUMMVM_BUILD_FROM_SOURCE=1`).** The engine is compiled from the root `scummvm/` submodule. Required when changing engine code, overrides, platform glue or the engine build flags — and used by the release pipeline itself.
...
Android-specific setup, API, native-build details, and limitations are documented in [`android/README.md`](../android/README.md).
