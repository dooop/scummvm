...
A thin SwiftUI wrapper around the upstream ScummVM codebase, packaged as a Swift Package. Reuse as much upstream C/C++ as possible via a git submodule; keep wrapper changes minimal and localized to Swift/ObjC++ glue code.

## Build modes

`Package.swift` reads `SCUMMVM_BUILD_FROM_SOURCE` at manifest-load time and switches the whole engine graph:

- **Binary mode (default)** — `ScummVMiOS`/`ScummVMmacOS` resolve to prebuilt dynamic-framework XCFrameworks downloaded from `engineBinaryBaseURL`. The frameworks absorb the third-party static libraries, link the system frameworks themselves and carry the runtime payload in their own `Resources`, so the consumer graph is two artifacts and no submodule. This is what every downstream repo gets.
...
CI (`.github/workflows/ci.yml`) runs two tiers on GitHub-hosted `macos-15` runners: a *binary build* on macOS/iOS/tvOS that exercises exactly what consumers get (plus a symbol and resource smoke test on macOS), and a *source build* that only runs when engine-facing paths changed — `Package.swift`, `Sources/ScummVMEngineOverrides/`, `Sources/ScummVMiOS/`, `Sources/ScummVMmacOS/`, `Scripts/`, the submodule pointer — or on pushes to `main`.
...
- All changes must live in wrapper/glue code or `Package.swift`.
...
- Keep the public API surface small and stable: `ScummVM`, `ScummVMView`, `ScummVMEngine`. Don't add new public API without an explicit request.
...
- **`ScummVMiOS`** / **`ScummVMmacOS`** — platform glue exposing a public `ScummVMEngine.h` Objective-C API (`include/ScummVMEngine.h`) that Swift calls into. ObjC++ targets in source mode; prebuilt dynamic-framework XCFrameworks in binary mode. The framework bundle name *is* the Swift module name, which is why there are two XCFrameworks (iOS+tvOS slices, macOS slice) rather than one.
- **`ScummVMtvOS`** (Swift) — thin `@_exported import ScummVMiOS` re-export. Not a copy of the iOS glue; has no ObjC++ of its own.
