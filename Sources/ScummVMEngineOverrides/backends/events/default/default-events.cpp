...
bool DefaultEventManager::pollEvent(Common::Event &event) {
	...
	if (g_scummVMIOSWrapperForceQuitRequested.load(std::memory_order_relaxed)) {
		// Embedded iOS wrapper stop requests must not be re-routed through engine-specific
		// quit prompts. Mark the event manager as quitting before dispatching any queued
		// events so engine UI code never enters a hidden quit confirmation flow.
		if (!g_engine) {
			// Launcher mode: exit the modal GUI loop directly so scummvm_main can unwind.
