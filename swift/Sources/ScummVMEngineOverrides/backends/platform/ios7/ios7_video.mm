...
static UIImage *scummVMIOSImageFromPDF(NSString *pdfPath) {
	CGPDFDocumentRef document = CGPDFDocumentCreateWithURL((CFURLRef)[NSURL fileURLWithPath:pdfPath]);
	if (document == nil) {
		return nil;
	}
	...
	if (CGRectIsEmpty(mediaBox) || mediaBox.size.width <= 0.0f || mediaBox.size.height <= 0.0f) {
		CGPDFDocumentRelease(document);
		return nil;
	}

	UIGraphicsBeginImageContextWithOptions(mediaBox.size, false, 0.0f);
	...
	// Quartz coordinates are flipped versus UIKit.
	CGContextTranslateCTM(context, 0.0f, mediaBox.size.height);
...
static UIImage *scummVMIOSImageNamedInBundle(NSString *name, NSBundle *resourceBundle) {
	...
	// Fallback for builds that copied the raw xcassets directory instead of processing it.
	NSString *imageSetDirectory = [NSString stringWithFormat:@"Images.xcassets/%@.imageset", name];
	NSString *pdfPath = [resourceBundle pathForResource:[name stringByAppendingString:@".pdf"] ofType:nil inDirectory:imageSetDirectory];
	if (pdfPath.length > 0) {
		image = [UIImage imageWithContentsOfFile:pdfPath];
		if (image == nil) {
			image = scummVMIOSImageFromPDF(pdfPath);
		}
		if (image != nil) {
			return image;
		}
...
static UIImage *scummVMIOSImageNamed(NSString *name) {
	UIImage *image = [UIImage imageNamed:name];
	if (image != nil) {
		return image;
	}

	image = scummVMIOSImageNamedInBundle(name, [NSBundle mainBundle]);
	if (image != nil) {
		return image;
	}

	image = scummVMIOSImageNamedInBundle(name, scummVMIOSModuleResourceBundle());
	if (image != nil) {
		return image;
	}
...
#endif
#if TARGET_OS_TV
	CGPoint _tvTouchRawPoint;
	CGPoint _tvTouchFilteredPoint;
	BOOL _tvTouchHasPoint;
	BOOL _tvTouchIgnoreNextDelta;
#endif
}
...
- (id)initWithFrame:(struct CGRect)frame {
	...
	UIImage *menuBtnImage = scummVMIOSImageNamed(@"ic_action_menu");
	...
	UIImage *toggleTouchModeBtnImage = scummVMIOSImageNamed(@"ic_action_touchpad");
...
#if TARGET_OS_TV
	[self resetTVTouchSmoothingState];
#endif
...
#if TARGET_OS_TV
- (void)resetTVTouchSmoothingState {
	_tvTouchHasPoint = NO;
	_tvTouchRawPoint = CGPointZero;
	_tvTouchFilteredPoint = CGPointZero;
	_tvTouchIgnoreNextDelta = NO;
}

- (CGPoint)scaledTVTouchLocation:(UITouch *)touch {
...
- (CGPoint)clampedTVTouchLocation:(CGPoint)point {
	CGFloat scale = [self contentScaleFactor];
	CGFloat maxX = self.bounds.size.width * scale;
	CGFloat maxY = self.bounds.size.height * scale;
...
- (CGPoint)smoothedTVTouchLocationForTouch:(UITouch *)touch {
	const CGFloat kRemoteSensitivity = 0.4f;

	CGPoint rawPoint = [self clampedTVTouchLocation:[self scaledTVTouchLocation:touch]];
	if (!_tvTouchHasPoint) {
		_tvTouchRawPoint = rawPoint;
		_tvTouchFilteredPoint = rawPoint;
		_tvTouchHasPoint = YES;
		_tvTouchIgnoreNextDelta = YES;
		return _tvTouchFilteredPoint;
	}

	CGPoint rawDelta = CGPointMake(rawPoint.x - _tvTouchRawPoint.x, rawPoint.y - _tvTouchRawPoint.y);
	_tvTouchRawPoint = rawPoint;
	if (_tvTouchIgnoreNextDelta) {
		_tvTouchIgnoreNextDelta = NO;
		return _tvTouchFilteredPoint;
	}
	_tvTouchFilteredPoint.x += rawDelta.x * kRemoteSensitivity;
	_tvTouchFilteredPoint.y += rawDelta.y * kRemoteSensitivity;
	_tvTouchFilteredPoint = [self clampedTVTouchLocation:_tvTouchFilteredPoint];
...
- (void)updateTouchMode {
	...
	if (currentTouchMode == kTouchModeDirect) {
		btnImage = scummVMIOSImageNamed(@"ic_action_mouse");
	} else if (currentTouchMode == kTouchModeTouchpad) {
		btnImage = scummVMIOSImageNamed(@"ic_action_touchpad");
	} else {
		return;
	}
...
#if TARGET_OS_IOS
	[_toggleTouchModeButton setImage:scummVMIOSImageNamed(@"ic_action_keyboard") forState:UIControlStateNormal];
#endif
}
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event {
	...
	UITouch *touch = [touches anyObject];
	if (touch != nil) {
		CGPoint point = [self smoothedTVTouchLocationForTouch:touch];
		[self addEvent:InternalEvent(kInputTouchBegan, (int)point.x, (int)point.y)];
	}
	return;
#endif
	for (GameController *c : _controllers) {
...
- (void)touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event {
	...
	UITouch *touch = [touches anyObject];
	if (touch != nil) {
		BOOL hadPoint = _tvTouchHasPoint;
		CGPoint previousPoint = _tvTouchFilteredPoint;
		CGPoint point = [self smoothedTVTouchLocationForTouch:touch];
		if (!hadPoint) {
			[self addEvent:InternalEvent(kInputTouchBegan, (int)point.x, (int)point.y)];
		} else if ((int)previousPoint.x != (int)point.x || (int)previousPoint.y != (int)point.y) {
			[self addEvent:InternalEvent(kInputTouchMoved, (int)point.x, (int)point.y)];
		}
	}
	return;
#endif
