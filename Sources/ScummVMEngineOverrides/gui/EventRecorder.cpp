...
// Free function, not a EventRecorder member: upstream's header only declares
// isImGuiRecorderEnabled() inside #ifdef USE_IMGUI, but every call site below
// (and the out-of-line definition further down) needs it available
// unconditionally. Forward-declared here since the first call site precedes
// the definition.
static bool isImGuiRecorderEnabled();
