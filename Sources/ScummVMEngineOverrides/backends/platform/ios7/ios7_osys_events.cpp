...
extern std::atomic<bool> g_scummVMIOSWrapperForceQuitRequested;
...
bool OSystem_iOS7::pollEvent(Common::Event &event) {
	...
	if (g_scummVMIOSWrapperForceQuitRequested.load(std::memory_order_relaxed)) {
		// Wrapper-forced shutdown must bypass launcher redirection and quit confirmation,
		// otherwise the engine can remain alive in a hidden launcher/dialog state.
		...
		if (g_system && g_system->getEventManager()) {
			// Clear stale flags from a prior launcher/game loop iteration so the forced
			// quit event is handled as a fresh quit and not preserved as "return to launcher".
			g_system->getEventManager()->resetReturnToLauncher();
			g_system->getEventManager()->resetQuit();
		}

		if (g_engine && !Engine::shouldQuit()) {
			// Ensure engine-local quit loops notice the shutdown request even if they
			// are not processing backend events promptly.
			Engine::quitGame();
		}

		// If the engine has already unwound to the launcher, force the launcher modal loop
		// to exit instead of idling hidden in the embedded host.
		if (!g_engine) {
			ConfMan.setActiveDomain(Common::String());
