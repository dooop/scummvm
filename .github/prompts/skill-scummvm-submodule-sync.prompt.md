---
mode: agent
description: Update and reconcile the upstream ScummVM submodule for this wrapper. Use when advancing Sources/ScummVMEngine to a newer upstream commit, reviewing override drift in Sources/ScummVMEngineOverrides, or cleaning stale Package.swift exclusions after upstream file churn.
---
...
1. Record the current upstream SHA with `git -C Sources/ScummVMEngine rev-parse HEAD`.
2. Advance `Sources/ScummVMEngine` to the requested upstream commit or branch.
3. Capture upstream file churn with `git -C Sources/ScummVMEngine diff --name-status <old-sha> HEAD`.
