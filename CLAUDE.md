...
This file provides guidance to Claude Code (claude.ai/code) when working in the cross-platform `scummvm` wrapper repository.
...
`ScummVMApp` (`swift/Sources/ScummVMApp/`) is a minimal macOS executable target useful for manually exercising the `ScummVM` SwiftUI view during development — build/run it directly for a quick end-to-end check on macOS.

Build the Android AAR (needs JDK 17+, the Android SDK, and NDK r28 or newer; the repository default is in `gradle.properties`):
```sh
./gradlew :scummvm:assembleRelease
```

CI (`.github/workflows/ci.yml`) runs two Apple tiers on GitHub-hosted `macos-15` runners: a *binary build* on macOS/iOS/tvOS that exercises exactly what consumers get (plus a symbol and resource smoke test on macOS), and a *source build* that only runs when engine-facing paths changed — `Package.swift`, `swift/Sources/ScummVMEngineOverrides/`, `swift/Sources/ScummVMiOS/`, `swift/Sources/ScummVMmacOS/`, `swift/Scripts/`, the submodule pointer — or on pushes to `main`. The Android AAR is built on `ubuntu-latest` and always runs, since Android has no prebuilt-binary mode to fall back on.
...
- `swift/Sources/ScummVMEngine` is a tracked compatibility symlink to `../../scummvm`; treat it as the same read-only upstream tree. SwiftPM needs the alias to keep Xcode native targets scoped under `swift/Sources/`.
- All changes must live in wrapper/glue code, `Package.swift`, or `android/`.
- If a build issue requires changing upstream source, add a replacement translation unit under `swift/Sources/ScummVMEngineOverrides/` at the mirrored path and exclude the original upstream file in `Package.swift`'s `exclude` list. This is the *only* sanctioned way to alter engine behavior.
...
- `swift/Sources/ScummVM/` — SwiftUI wrapper (Swift)
- `swift/Sources/ScummVMiOS/`, `swift/Sources/ScummVMmacOS/`, `swift/Sources/ScummVMtvOS/` — platform glue (ObjC++/Swift)
- `swift/Sources/ScummVMEngineOverrides/` — replacement translation units for upstream build fixes
- `swift/Scripts/` — engine XCFramework build/packaging scripts
...
2. Add a replacement translation unit at the mirrored path under `swift/Sources/ScummVMEngineOverrides/` (see existing examples: `common/recorderfile.cpp`, `gui/onscreendialog.cpp`, `gui/recorderdialog.cpp`, `base/base_main_override.cpp`, `engines/plugins_table.h`, `engines/detection_table.h`).
...
- **`:scummvm`** (`com.android.library`) — public Compose API in `org.scummvm`; glue that needs upstream package-private access lives in `org.scummvm.scummvm`.
- **`:app`** (`com.android.application`) — sample and consumer-validation app. Debug consumes the local project; release consumes the AAR selected by `scummvm.releaseAar`.
...
- Toolchain pins use AGP 9.2.1, Gradle 9.6.1, and NDK 29.0.14206865 by default. NDK r28+ is required for 16 KB page compatibility; the wrapper relaxes only upstream's obsolete exact-version check in a build-local copy of `configure`.
...
| Adding/updating an override in `swift/Sources/ScummVMEngineOverrides/` | `scummvm-override-workflow` |
| Editing lifecycle/bridge code in `swift/Sources/ScummVM/`, `ScummVMiOS/`, `ScummVMmacOS/`, `ScummVMtvOS/` | `objcxx-bridge-lifecycle` |
...
| Android Compose API, JNI host, input, or lifecycle changes | `android-compose-development` |
| Android Gradle, Kotlin, NDK, native, JNI, AAR, or sample build failures | `android-build-triage` |
| Android AAR/APK release and consumer validation | `android-release-checks` |
