...
// Wrapper-specific: allow a host-requested stop to terminate the engine loop
// instead of returning to the launcher (default iOS backend behavior).
std::atomic<bool> g_scummVMIOSWrapperForceQuitRequested(false);
std::atomic<bool> g_scummVMIOSWrapperTimerCallbacksEnabled(false);

static dispatch_source_t g_scummVMIOSDispatchTimer = nullptr;
static OSystem_iOS7 *g_scummVMIOSSharedInstance = nullptr;

static void iOS7_cancelDispatchTimer() {
	if (!g_scummVMIOSDispatchTimer)
		return;

	dispatch_source_t timer = g_scummVMIOSDispatchTimer;
	g_scummVMIOSDispatchTimer = nullptr;
...
OSystem_iOS7::~OSystem_iOS7() {
	...
	iOS7_cancelDispatchTimer();

	if (s_AudioQueue.queue) {
		AudioQueueDispose(s_AudioQueue.queue, true);
		s_AudioQueue.queue = nullptr;
	}

	delete _mixer;
	_mixer = nullptr;
	// _graphicsManager is owned by ModularGraphicsBackend.

	if (g_scummVMIOSSharedInstance == this)
		g_scummVMIOSSharedInstance = nullptr;
}
...
int OSystem_iOS7::timerHandler(int t) {
	if (!g_scummVMIOSWrapperTimerCallbacksEnabled.load(std::memory_order_relaxed))
		return t;
...
void OSystem_iOS7::initBackend() {
	g_scummVMIOSWrapperForceQuitRequested.store(false, std::memory_order_relaxed);
	...
	g_scummVMIOSWrapperTimerCallbacksEnabled.store(true, std::memory_order_relaxed);
...
bool OSystem_iOS7::hasFeature(Feature f) {
	...
	case kFeatureNoQuit:
		// The Swift wrapper embeds ScummVM as a session that must be able to exit cleanly
		// from both games and the launcher. Reporting "no quit" traps launcherDialog() in a
		// modal loop because it caches this flag before entering the loop.
		return false;
...
void OSystem_iOS7::suspendLoop() {
	...
	InternalEvent event;
	while (!done) {
		if (g_scummVMIOSWrapperForceQuitRequested.load(std::memory_order_relaxed))
			break;
...
void OSystem_iOS7::setTimerCallback(TimerProc callback, int interval) {
	...
	if (timer)
	{
		...
		g_scummVMIOSDispatchTimer = timer;
...
void OSystem_iOS7::quit() {
	g_scummVMIOSWrapperForceQuitRequested.store(true, std::memory_order_relaxed);
}
...
OSystem_iOS7 *OSystem_iOS7::sharedInstance() {
	if (!g_scummVMIOSSharedInstance)
		g_scummVMIOSSharedInstance = new OSystem_iOS7();
	return g_scummVMIOSSharedInstance;
}
