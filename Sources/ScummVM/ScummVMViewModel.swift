...
final class ScummVMViewModel: ObservableObject {
  ...
  func start() {
    ...
    startTask = Task {
      let resolvedPath = await pathResolver.resolveGamePath(requestedGamePath)

      guard !Task.isCancelled else {
        return
      }

      ScummVMEngineSharedInstance().start(gamePath: resolvedPath)
      startTask = nil
    }
  }
...
private actor ScummVMGamePathResolver {
  private static let supportedArchiveExtensions: Set<String> = ["zip", "scummvm"]

  func resolveGamePath(_ gamePath: String?) async -> String? {
    guard let gamePath else { return nil }

    let sourceURL = URL(fileURLWithPath: gamePath)
    ...
    do {
      ...
      return extractedDirectory.path
    } catch {
  ...
  private func extractArchive(at archiveURL: URL, using fileManager: FileManager) throws -> URL {
  ...
  #if os(iOS) || os(tvOS)
    private func importDirectoryIfNeeded(at sourceURL: URL, using fileManager: FileManager) throws -> URL {
    ...
    private func isLaunchableInPlaceOnSandboxedPlatforms(_ sourceURL: URL, using fileManager: FileManager)
      throws -> Bool
    {
  ...
  private func importedContentRootDirectory(named leafDirectory: String, using fileManager: FileManager)
    throws -> URL
  {
    #if os(iOS) || os(tvOS)
  ...
  private func preferredGameDirectory(in extractionDirectory: URL, using fileManager: FileManager) throws
    -> URL
  {
